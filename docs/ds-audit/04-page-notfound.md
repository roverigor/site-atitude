...
- **Estado geral:** YELLOW (3 resolved during rollout, 1 still-deferred as new-feature)
...
- 🟡 **No entry animation:** ObrigadoContent uses `motion.div` with fade+slide for its hero block. A 404 page with a static layout is acceptable, but an entrance fade (opacity 0→1, y 20→0) would match site convention. **Still deferred:** new-feature (framer-motion entrance), out of scope for DS-compliance polish PR.
