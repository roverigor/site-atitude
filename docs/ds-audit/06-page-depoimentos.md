...
- 🟡 "Destaques" section label `h2` at TestimonialsPage.tsx:202 uses `text-lg font-semibold` — this is a section label not a content heading; `.eyebrow` would change semantics; acceptable as label style — Fix: none (deferred, structural concern) — **Still deferred:** structural — section label semantics differ from content heading; .eyebrow would change meaning
...
- 🟡 `bg-white` on FeaturedCard (line 42) and TestimonialCard (line 102) — DS has `--color-paper: #FFFFFF` but `bg-white` is the v4 alias; functionally identical in light mode; dark mode handled via `dark:bg-[var(--color-background-alt)]` — Fix: none (deferred) — **Still deferred:** bg-white is Tailwind 4 alias for --color-paper (functionally identical); dark mode already uses --color-background-alt
...
- 🟡 `TestimonialCard` framer-motion prop `transition={{ duration: 0.2 }}` at TestimonialsPage.tsx:101 — 0.2s = 200ms, not referencing DS token; framer-motion props cannot reference CSS vars directly, inline numeric value is acceptable — Fix: none (deferred) — **Still deferred:** framer-motion props cannot reference CSS vars; inline numeric is accepted constraint
